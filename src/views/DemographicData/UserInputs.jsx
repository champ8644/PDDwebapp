...
const validateHoles = {
  mobilePhone: [/\d/, /\d/, /\d/, /\d/, /\d/, /\d/, /\d/, /\d/, /\d/, /\d/],
};

// hole = [/rule/, "-"]
function validateInPlace(text, hole){
  let chars = text.split('');
  for(let i=0; i<chars.length; i++){
    let re = hole[i];
    if(re != undefined && re.test(chars[i])){
      continue;
    }
    else{
      return false;
...
function validateAction(action){  
  if(!(action.fieldName in validateHoles)){
    return true;
  }else{
    let hole = validateHoles[action.fieldName];
    return validateInPlace(action.fieldValue, hole);
  }
}
...
function reducer(state, action){
  switch (action.type) {
    case 'CHANGE':
      var valid = validateAction(action);
      if(valid){
        return {
          ...state, 
          [action.fieldName]: {value: action.fieldValue, error: null}
        };
      }else{
        return state;
      }
